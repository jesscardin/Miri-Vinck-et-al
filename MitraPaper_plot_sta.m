function [] = MitraPaper_plot_sta(dirsel)
...
%  fileNames = fileNames(selection);
 ...
 [N,B] = histc(spike_base.timestamp{1}, dataLfp.time_timestamp{1});
  ...
  for iChannel = 1:length(sta.label)
      ...
      figure, 
      plot(freq.freq, log10(freq.powspctrm(iChannel,:)))
      xlabel('frequency')
      ylabel('log 10 power')
      filename = fullfile(outputDir, b, info(iDir).name, [sta.label{iChannel} 'spectrum.jpeg']);
      saveas(gcf,filename)
