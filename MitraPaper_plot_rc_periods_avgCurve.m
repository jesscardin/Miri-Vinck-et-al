...
%keyboard
                valBase = NaN(length(sl),1,length(levs));
                for iN = find(sl)'
                    for k = 1:length(levs)
                        idx = nearest(10.^R.levAllBase(iN,:),levs(k));
                        if abs(10.^R.levAllBase(iN,idx)-levs(k))>0.01, continue,end
                        valBase(iN,1,k) = R.fitBase(iN,idx);
                    end
                end

                valIctal = NaN(length(sl),4,length(levs));
                for iN = find(sl)'
                    for k = 1:length(levs)
                        for iIctal = 1:nIctalPeriods
                            idx = nearest(squeeze(10.^R.levAllIctal(iN,iIctal,:)),levs(k));
                         %   keyboard
                            if abs(squeeze(10.^R.levAllIctal(iN,iIctal,idx))-levs(k))>0.01, continue,end
                       %     keyboard
                            valIctal(iN,iIctal,k) = squeeze(R.fitIctal(iN,iIctal,idx));
                        end
                    end
                end
                dat = cat(2,valBase(sl,1,:),valIctal(sl,end,:));
            %    keyboard
                
                NN = repmat(sum(~isnan(dat),2),[1 2 1]);
                dat(NN<2) = NaN;
               % keyboard
                sll = repmat(all(~isnan(dat),3), [1 1 size(dat,3)]);
                dat(~sll) = NaN;
                mn = squeeze(nanmean(dat,1));
                sm = squeeze(nansem(dat,1));
                n = squeeze(sum(~isnan(dat)));
                figure(iType*3+2), plot(levs,n')
                
                cors = {'k', 'r', 'g', 'c', 'b'}
                
                figure(iType*3)
                for iIctal = 1:2
                    plotmeansem(levs,mn(iIctal,:),sm(iIctal,:),sm(iIctal,:),[cors{iIctal} '-'],[cors{iIctal} '--'])
                    hold on
                end
                
                figure(iType*3+1)
                for iIctal = 1:2
                    plot(levs,mn(iIctal,:),cors{iIctal})
                    hold on
                end
            end
