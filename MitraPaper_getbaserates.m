...
for nIctalPeriods = 4
    ...
    for iFile = 1:nFiles
        ...
        if info(iFile).has_rc==0
            ...
            [TimeStamp,B,C,D,data,F] = Nlx2MatSpike(fullfile(info(iFile).directory, 'optopulses.ntt'), fieldSelection,extractHeader, 1);
             ...
             rate_base(iFile) = MitraPaper_rate_opto(fullfile(info(iFile).directory, info(iFile).name), TimeStamp, info(iFile).nlevels,info(iFile).base_start_pulse, info(iFile).base_end_pulse,0.1);
    ...
    outputdir = fullfile(datadir_manuscript, 'average_base_rates');
    mkdir(outputdir)
    str = ['average_base_rate_' num2str(nIctalPeriods)];
